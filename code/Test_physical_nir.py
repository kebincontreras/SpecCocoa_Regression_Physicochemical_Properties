...
# === CONFIGURACIÓN ===
DATA_PATH = "data/TEST_test_nir_cocoa_dataset.h5"
NORM_FACTORS = {
    'Cadmium': 5.6,
...
def load_best_ml_model_nir():
    model_dir = "model/Machine_Learning/NIR/"
    best_r2 = -np.inf
    ...
    for file in os.listdir(model_dir):
        if file.endswith(".pkl"):
            try:
                ...
                continue
    if best_model_path:
        print(f"✅ ML modelo NIR: {os.path.basename(best_model_path)}")
        with open(best_model_path, "rb") as f:
            return pickle.load(f)
    raise FileNotFoundError("❌ No hay modelo ML NIR")

# === CARGAR MEJOR MODELO DL ===
def load_best_dl_model_nir(num_bands, num_outputs, device):
    model_dir = "model/Deep_Learning/NIR/"
    best_r2 = -np.inf
    best_model_path = None
    for file in os.listdir(model_dir):
    ...
    if not best_model_path:
        raise FileNotFoundError("❌ No hay modelo DL NIR")

    print(f"✅ DL modelo NIR: {os.path.basename(best_model_path)}")
...
def run_predictions_nir():
    ...
    model_ml = load_best_ml_model_nir()
    model_dl = load_best_dl_model_nir(num_bands, num_outputs, device)
    ...
    print("\n📊 Comparación por Lote (NIR - Machine Learning)")
    print(tabulate(tabla_ml, headers=headers, tablefmt="grid"))

    print("\n📊 Comparación por Lote (NIR - Deep Learning)")
